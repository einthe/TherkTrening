...
alter table public.user_component_instances
  add column show_on_analysis boolean not null default false;
comment on column public.user_component_instances.enabled is 'Visibility on Dashboard.';
comment on column public.user_component_instances.show_on_analysis is 'Visibility on Analysis, independent of Dashboard.';
...
begin
  ...
  if action='createEvents' then
    ...
    if item ? 'showOnAnalysis' and jsonb_typeof(item->'showOnAnalysis') is distinct from 'boolean' then raise exception 'Invalid Analysis visibility'; end if;
    ...
    if found then
      ...
      update public.user_component_instances set title=item->>'title',enabled=(item->>'enabled')::boolean,show_on_analysis=coalesce((item->>'showOnAnalysis')::boolean,inst.show_on_analysis),position=(item->>'position')::integer,config=item->'config',updated_at=stamp where id=inst.id;
    else
      ...
      insert into public.user_component_instances(id,user_id,component_definition_id,version,title,enabled,show_on_analysis,position,config) values((item->>'id')::uuid,auth.uid(),item->>'componentDefinitionId',1,item->>'title',(item->>'enabled')::boolean,coalesce((item->>'showOnAnalysis')::boolean,false),(item->>'position')::integer,item->'config');
    end if;
