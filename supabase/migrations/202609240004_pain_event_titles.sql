...
-- Name historical check-ins only when their injuries match exactly one current card.
-- Keep dates, readings, notes and lock states unchanged.
with matches as (
  select e.id, min(c.title) as title
  from public.events e join public.user_component_instances c on c.user_id=e.user_id and c.component_definition_id='pain_logger'
  where e.event_type='pain_measurement' and e.schema_version=1 and not e.payload ? 'name'
    and not exists(
      select 1 from jsonb_array_elements(case when e.payload ? 'readings' then e.payload->'readings' else jsonb_build_array(e.payload) end) r
      where not coalesce(c.config->'targets','[]'::jsonb) ? (r->>'injuryId')
    )
  group by e.id having count(*)=1
)
update public.events e set payload=e.payload || jsonb_build_object('name',matches.title),updated_at=clock_timestamp()
from matches where e.id=matches.id;
